module.exports.up = async (sequelize) => {
  const entries = [
    { user_email: 'alice@example.com', project_code: 'P-2024-001', privs: ['CREATE','READ','UPDATE','DELETE'] },
    { user_email: 'bob@example.com', project_code: 'P-2024-002', privs: ['READ','UPDATE'] },
  ];
  ...
  for (const e of entries) {
    ...
    for (const pname of e.privs) {
      ...
      await sequelize.query(
        'INSERT INTO `user_privilege` (`privilege_id`, `project_id`, `user_id`) SELECT ?, ?, ? FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `user_privilege` WHERE `privilege_id` = ? AND `project_id` = ? AND `user_id` = ?)',
...
module.exports.down = async (sequelize) => {
  ...
  for (const u of users) {
    ...
    if (user) {
      await sequelize.query('DELETE FROM `user_privilege` WHERE `user_id` = ?', { replacements: [user.id] });
    }
  }
};
